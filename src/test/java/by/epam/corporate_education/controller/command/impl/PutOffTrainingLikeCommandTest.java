...
public class PutOffTrainingLikeCommandTest {
    ...
    public void init() {
        ...
        command = new PutOffTrainingLikeCommand(userService, utilFactory);
    }

    @Test(expected = CommandException.class)
    public void execute_serviceExceptionFromPutOffLike_commandException_errorPath() throws ServiceException, CommandException {
        //given
        //when
        ...
        Mockito.doThrow(ServiceException.class).when(userService).putOffLike(Mockito.anyInt(), Mockito.anyInt());
    ...
    public void execute_validParameters_viewTrainingPath() throws ServiceException, CommandException {
        ...
        Mockito.when(request.getSession()).thenReturn(session);
        Mockito.when(session.getAttribute(ParameterName.ID_USER)).thenReturn(1);
        Mockito.doNothing().when(userService).putOffLike(Mockito.anyInt(), Mockito.anyInt());
        String result = command.execute();
        //then
