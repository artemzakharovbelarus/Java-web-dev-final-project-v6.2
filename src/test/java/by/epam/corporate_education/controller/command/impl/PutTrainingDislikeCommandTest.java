...
public class PutTrainingDislikeCommandTest {
    ...
    public void init() {
        ...
        command = new PutTrainingDislikeCommand(userService, utilFactory);
    }

    @Test(expected = CommandException.class)
    public void execute_serviceExceptionFromAddTrainingDislike_commandException_errorPath()
        ...
        Mockito.doThrow(ServiceException.class).when(userService).addTrainingDislike(Mockito.anyInt(), Mockito.anyInt());
    ...
    public void execute_validParameters_viewTrainingPath()
        ...
        Mockito.when(request.getSession()).thenReturn(session);
        Mockito.when(session.getAttribute(ParameterName.ID_USER)).thenReturn(1);
        Mockito.doNothing().when(userService).addTrainingDislike(Mockito.anyInt(), Mockito.anyInt());
        String result = command.execute();
        //then
