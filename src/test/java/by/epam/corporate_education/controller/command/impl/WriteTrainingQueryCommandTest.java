...
public class WriteTrainingQueryCommandTest {
    ...
    public void init(){
        ...
        command = new WriteTrainingQueryCommand(userService, utilFactory);
    }

    @Test(expected = CommandException.class)
    public void execute_serviceExceptionFromWriteQuery_commandException_errorPath() throws ServiceException, CommandException {
        //given
        //when
        ...
        Mockito.doThrow(ServiceException.class).when(userService).writeQuery(Mockito.anyInt(), Mockito.anyInt());
    ...
    @Test
    public void execute_validParameter_viewTrainingPath() throws ServiceException, CommandException {
        //given
        //when
        Mockito.when(request.getParameter(ParameterName.ID_TRAINING)).thenReturn("1");
        HttpSession session = Mockito.mock(HttpSession.class);
        Mockito.when(request.getSession()).thenReturn(session);
        Mockito.when(session.getAttribute(ParameterName.ID_USER)).thenReturn(1);
        Mockito.doNothing().when(userService).writeQuery(Mockito.anyInt(), Mockito.anyInt());
        String result = command.execute();
        //then
        String expected = pathCreator.getEnrolTraining(request.getContextPath(), 1);
        assertEquals(expected, result);
    }
}