...
public class PutOffTrainingDislikeCommandTest {
    ...
    public void init(){
        ...
        command = new PutOffTrainingDislikeCommand(userService, utilFactory);
    }

    @Test(expected = CommandException.class)
    public void execute_serviceExceptionFromPutOffDislike_commandException_errorPath() throws ServiceException, CommandException {
        //given
        //when
        ...
        Mockito.doThrow(ServiceException.class).when(userService).putOffDislike(Mockito.anyInt(), Mockito.anyInt());
    ...
    public void execute_validParameter_ViewTrainingPath() throws ServiceException, CommandException {
        ...
        Mockito.when(request.getSession()).thenReturn(session);
        Mockito.when(session.getAttribute(ParameterName.ID_USER)).thenReturn(1);
        Mockito.doNothing().when(userService).putOffDislike(Mockito.anyInt(), Mockito.anyInt());
        String result = command.execute();
        //then
